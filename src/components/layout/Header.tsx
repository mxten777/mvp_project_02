  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur shadow-sm border-b border-slate-200 dark:border-slate-800 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 flex items-center justify-between h-16">
        <Link href="/" className="flex items-center gap-2 group py-2.5 px-1">
        ...
        <nav className="hidden md:flex lg:hidden gap-6 text-base font-medium">
          <Link href="/" className="hover:text-blue-600 transition py-2.5 px-2">홈</Link>
          <Link href="/order" className="hover:text-blue-600 transition py-2.5 px-2">주문</Link>
          <Link href="/admin" className="hover:text-blue-600 transition py-2.5 px-2">관리자</Link>
          <Link href="#contact" className="hover:text-blue-600 transition py-2.5 px-2">문의</Link>
        </nav>
        {/* PC 메가메뉴: lg~ */}
        <PcMegaMenu />
        <div className="flex items-center gap-2">
          ...
          <DarkModeToggle />
          <Link href="#contact" className="ml-2 px-4 py-2.5 rounded bg-blue-600 text-white font-semibold shadow hover:bg-blue-700 transition hidden sm:inline-block min-w-[90px] text-center">
            상담 문의
          </Link>
          {/* 모바일 메뉴 버튼 - 터치 영역 확대 */}
